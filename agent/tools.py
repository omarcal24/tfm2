...
class MockBookingSystem:
    ...
    def make_booking(self, place_id: str, name: str, date: str, 
                    time: str, num_people: int) -> Dict:
...
def phone_call(
    ...
    """
    phone_number = "+34649122018"  # Para pruebas Alejandro
    # phone_number = "+34699232329"  # Para pruebas Carla
    # phone_number = "+34636760453"  # Para pruebas Fabio
    # phone_number = "+34696743410"  # Para pruebas Mama

    
    CALL_SERVICE_URL = "http://localhost:8080"
    
    # Verificar servicio disponible
    try:
        health = requests.get(f"{CALL_SERVICE_URL}/", timeout=5)
        if health.status_code != 200:
            return "ERROR: El servicio de llamadas no está disponible. Ejecuta: python run.py call"
    except requests.exceptions.ConnectionError:
        return "ERROR: No se pudo conectar al servicio de llamadas (puerto 8080). ¿Está corriendo?"
    ...
    while time_module.time() - start_time < max_wait:
        try:
            status_response = requests.get(
                f"{CALL_SERVICE_URL}/call-status/{call_id}",
                timeout=5
            )
    ...
    return f"⏱️ La llamada está tardando más de lo esperado (>{max_wait}s). ID: {call_id}"
...
# Inicializamos el toolkit pero lo mantenemos privado para usarlo internamente
_calendar_toolkit = CalendarToolkit()
_google_tools = {t.name: t for t in _calendar_toolkit.get_tools()}
