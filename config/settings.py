...
def load_config():
    ...
    config = {
        ...
        "THRESHOLD_CV_FAITHFULNESS": float(
            os.getenv("THRESHOLD_CV_FAITHFULNESS", 0.5) or 0.5
        ),
        "THRESHOLD_EVALUATION_HALLUCINATION": float(
            os.getenv("THRESHOLD_EVALUATION_HALLUCINATION", 0.8) or 0.8
        ),
        "THRESHOLD_INTERVIEW_COMPLETENESS": float(
            os.getenv("THRESHOLD_INTERVIEW_COMPLETENESS", 0.8) or 0.8
        ),
        "THRESHOLD_INTERVIEW_FAITHFULNESS": float(
            os.getenv("THRESHOLD_INTERVIEW_FAITHFULNESS", 0.8) or 0.8
        ),
        "THRESHOLD_INTERVIEW_HALLUCINATION": float(
    ...
    if not config["GOOGLE_MAPS_API_KEY"]:
        raise ValueError(
            "GOOGLE_MAPS_API_KEY no está configurada. Añádela al archivo .env"
        )
